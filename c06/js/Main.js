...
function initWheel(){
	...
	mainCanvas.width = (wheelRadius+5)*2; //gameWidth;
	mainCanvas.height = (wheelRadius+5)*2+50; //gameHeight;
	...
	if(rawWheelData.length > 0){
		...
		for(var i=0; i<wheelSegment; i++){
			...
			if(thisMin != -1 && thisMax != -1) {
				var thisRange = thisMax - thisMin + 1;
				if(thisMin == 0) thisRange -= 1;
				// console.log(thisRange);
				wheelRange += thisRange; //(thisMax - thisMin) + 1
			};
...
function renderWheel() {
	...
	for(var i=0; i<wheelData.length; i++){
		drawPieSlice(wheelContext, wheelCanvas.width*0.5, wheelCanvas.height*0.5, wheelRadius-5, wheelData[i].startAngle, wheelData[i].angle, wheelData[i].color, wheelData[i].data);
	};
...
function drawPieSlice(ctx, centerX, centerY, radius, startAngle, endAngle, color, data ){
    ...
    wrapText(ctx, data, radius*0.6, 0, 120, 26);
    wrapText(ctx, data, radius*0.6, 0, 120, 26);
    ctx.translate(-wheelCanvas.width*0.5, -wheelCanvas.height*0.5);
    ctx.restore();
}
...
function luckyDraw() {
	if(clickOnce){
		...
		// var spincount = document.getElementById('spincount').value;
		...
		if(prizeAvailable > 0){
			...
			do {
				...
				if(wheelData[tempPrizeID].quantity > 0){
					...
					if(!wheelData[thisPrizeID].isLose) {
						
						if (spincount > 0) {
							wheelData[thisPrizeID].quantity--;
...
function displayPrizeModal(prizeID){
	if(wheelData[prizeID].isLose){
		...
		// $('.numberSpin').innerHTML = wheelData[prizeID].data;
...
function initSettings(){
	...
	for(var i=2; i<=maxSegment; i++){
		...
		segmentSelectorElem += '<li class="btn-segment-'+thisIndex+'"><a onclick="renderSegmentList('+i+')">'+i+'</a></li>';
	};
...
function renderSegmentList(count){
	var segmentSettingsElem = '<ul class="segment-settings-wrapper"><li class="segment-settings-label"><div class="row"><div class="col-xs-1">#</div><div class="col-xs-5">Prize Name</div><div class="col-xs-2 text-center">Quantity</div><div class="col-xs-2 text-center">Chances</div><div class="col-xs-2 text-center">Game Over</div></div></li>';
	...
	for(var i=1; i<=count; i++){
		...
		segmentSettingsElem += '<li class="segment-settings-'+thisIndex+'"><div class="row"><div class="col-xs-1">'+thisIndex+'</div><div class="col-xs-5"><input type="text" class="segment-name" placeholder="Prize name" value="'+thisName+'"></div><div class="col-xs-2"><input type="number" class="segment-quantity text-center" placeholder="Qty" value="'+thisQuantity+'"></div><div class="col-xs-2"><input type="number" class="segment-rate segment-id-'+i+' text-center" placeholder="Chances" value="'+thisChances+'"></div><div class="col-xs-2 text-center checkbox-wrapper"><input class="segment-gameover" type="checkbox" '+thisGameOver+'></div></div></li>';
	};
...
function constructWheelColor(){
	var colorList = ['#65afe1', '#d58543', '#edbd59', '#65afe1', '#b9c558', '#4f3b80', '#c54c80', '#65afe1', '#d58543', '#78a85e', '#edbd59'];
	var color = [];
